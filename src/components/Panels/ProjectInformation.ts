...
import { materialsList } from "../../bim-components/MaterialsManager/src/user-interface";

export default (components: OBC.Components) => {
  const [modelsList] = CUI.tables.modelsList({ components });
  const [materialsListTable] = materialsList({ components })
  ...
  return BUI.Component.create<BUI.Panel>(() => {
    return BUI.html`
      <bim-panel>
        ...
        <bim-panel-section label="Project Materials">
          ${materialsListTable}
        </bim-panel-section>
      </bim-panel> 
    `;
